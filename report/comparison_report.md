...
The most significant technical challenge was implementing CoAP's Observe pattern with concurrent subscriptions and stale notification detection. The observer client needed to simultaneously monitor temperature resources on both production lines using `asyncio.gather`, coordinating the observation loops with a 60-second cancellation timer. The stale notification detection added further complexity: CoAP Observe uses a 24-bit sequence number that can wrap around, so a naive comparison would incorrectly flag valid notifications. I resolved this by implementing the RFC 7641 freshness check, where a notification is stale only if its sequence number is lower than the last received AND the difference is less than 2^23 (half the sequence space). Cleanly cancelling both observation streams without leaving dangling tasks required explicitly cancelling the protocol request in a `finally` block.

### Most Surprising Protocol Difference

The most surprising difference observed during packet capture was the variation in per-message overhead. An MQTT PUBLISH for a temperature reading (~80 bytes payload) required only 6-8 bytes of protocol framing: the 2-byte fixed header, a topic length prefix, and the topic string. The equivalent CoAP response was similarly compact at roughly 12 bytes of header and options. However, the AMQP equivalent required three separate frames (Method, Content Header, and Body), each with its own frame type byte, channel number, payload size, and frame-end marker (0xCE). The total AMQP framing overhead exceeded 100 bytes, more than the payload itself, making concrete what textbooks describe abstractly about AMQP's feature-rich but heavier wire format.

### Most Complex Protocol to Implement

CoAP was the most complex protocol to implement. Unlike MQTT, where paho-mqtt and Mosquitto handle connection management and topic routing, CoAP required building the resource tree manually using aiocoap's `Resource` and `Site` classes, with each endpoint implementing `render_get`/`render_put` coroutines and correct response codes (2.05, 2.04, 4.00). Block2 transfer for the `/factory/manifest` resource was difficult to verify because reassembly happens transparently on the client side, and misconfiguration produces truncated responses with no error. Content-Format option codes had to be set explicitly on every response, unlike MQTT where payloads are opaque bytes. The HTTP-to-CoAP proxy added further complexity, requiring translation between HTTP headers and CoAP options within a shared asyncio event loop. Debugging was harder than MQTT since CoAP runs over UDP with no session state, so errors like malformed option deltas produced silent failures rather than connection exceptions.
